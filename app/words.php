...
$dsn = 'mysql:dbname=qryheavy_quiz; host=mysql57.qryheavy.sakura.ne.jp; charset=utf8';
$usr = 'qryheavy';
$passwd = '3exfc9Gn4n8xDp7';
...
(function() {
  ...
  var quizSet = [
    ...
    {r: <?= $cnt ?> + "/" + <?= $AllRow ?> , c:'<?= $row['category'] ?>' , k:'<?= $row['kbn'] ?>' , q:"<?= $row['question'] ?>" , a:"<?= $row['answer'] ?>" , h:"<?= $row['hint'] ?>" , i:'<?= $row['id'] ?>' },
  ...
  function setQuiz() {
    ...
    title.innerHTML = 'Stage<?= $grade ?>' + '  ' + quizSet[currentNum].r + ' id:' + quizSet[currentNum].i;
  ...
  function checkAnswer(node) {
    ...
    if (node.value === quizSet[currentNum].a) {
      ...
      Wrk = "<p style='color:#00F'>〇 問題:" + quizSet[currentNum].q + "　入力:" + escapeHtml(node.value) + "　答え:" + quizSet[currentNum].a + "　ヒント:" + quizSet[currentNum].h + "　ID:" + quizSet[currentNum].i + "</p>";
      score++;
    } else {
      //node.classList.add('wrong');
      Wrk = "<p style='color:#F00'>× 問題:" + quizSet[currentNum].q + "　入力:" + escapeHtml(node.value) + "　答え:" + quizSet[currentNum].a + "　ヒント:" + quizSet[currentNum].h + "　ID:" + quizSet[currentNum].i + "</p>" ;
    }
